...
for filename in files:
    ...
    try:
        c.execute("CREATE TABLE "+filename_we.upper()+" ('Season','Age','Tm','Lg','Pos','G','GS', 'MP','FG','FGA','FG%','3P','3PA','3P%','2P','2PA','2P%','FT','FTA','FT%','ORB','DRB','TRB','AST','STL','BLK','TOV','PF','PTS')")
        for row in creader:
            to_db = []
...
"""
#Code to drop tables if incorrect data uploaded. You need to comment the above code and run this
backup = "backup"
os.chdir(backup)
files = glob.glob("*.csv")
conn = sqlite3.connect("/MyData/Master'sProjects/Sem2/DataMining/Project/Data/NBAdataset.db")
